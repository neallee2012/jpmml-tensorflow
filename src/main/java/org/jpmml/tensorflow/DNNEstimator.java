  ...
  protected NeuralNetwork encodeNeuralNetwork(TensorFlowEncoder encoder) {
      ...
      for (int i = 0; i < inputNames.size() - 1; i++) {
        ...
        if (("Cast").equals(term.getOp()) || ("Placeholder").equals(term.getOp())) {
          ...
          if (("Sum").equals(term.getOp())) {
            ...
            List<BinaryFeature> binaryFeatures = encoder
                .createBinaryFeatures(savedModel, placeholder, categories);
    ...
    for (int i = 0; i < biasAdds.size(); i++) {
      ...
      for (int j = 0; j < count1; j++) {
        List<Float> entityWeights = CMatrixUtil
            .getColumn(Floats.asList(weightValues), (int) shape[0], (int) shape[1], j);
        Neuron neuron = NeuralNetworkUtil
            .createNeuron(entities, ValueUtil.floatsToDoubles(entityWeights),
                ValueUtil.floatToDouble(biasValues[j]))
